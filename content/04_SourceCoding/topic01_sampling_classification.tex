...
\begin{kbox}{抽样技术的分类体系}
    ...
    \begin{tikzpicture}[
        ...
        \node[root] {抽样}
            ...
            child {node[node1] {3. 按脉冲形状}
                child {node[node2] {实际抽样 (平顶/自然)}
                    node[right=0.2cm, font=\scriptsize, gray] {脉冲有宽度}
                }
                child {node[node2] {理想抽样 (冲激)}
                    node[right=0.2cm, font=\scriptsize, gray] {$\delta(t)$序列}
                }
            }
            % 分支 2：按时间间隔
            child {node[node1] {2. 按时间间隔}
                child {node[node2] {非均匀抽样 (随机)}}
                child {node[node2] {均匀抽样 (等间隔)}
                    node[right=0.2cm, font=\scriptsize, gray] {$T_s = \text{const}$}
                }
            }
            % 分支 1：按信号频谱
            child {node[node1] {1. 按信号频谱}
                child {node[node2] {带通抽样}
                     node[right=0.2cm, font=\scriptsize, gray] {对应带通信号}
                }
                child {node[node2] {低通抽样}
                     node[right=0.2cm, font=\scriptsize, gray] {对应基带信号}
                }
            };
    \end{tikzpicture}
    }
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item \textbf{低通抽样}：针对基带信号，满足 $f_s \ge 2f_H$ (本章重点)。
