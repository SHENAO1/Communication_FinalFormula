...
\begin{intuitionbox}{数学预备：从傅里叶级数到冲激串变换}
    ...
    \begin{equation}
        F_n = \frac{1}{T_s} \int_{\frac{-T_s}{2}}^{\frac{T_s}{2}} \delta(t) e^{-j n \omega_s t} dt \overset{\text{筛分性质}}{=} \frac{1}{T_s} \cdot 1 \cdot e^{0} = \frac{1}{T_s}
    \end{equation}
...
\begin{kbox}{理想抽样过程：波形与频谱对照}
    ...
    \begin{tikzpicture}[xscale=1.1, yscale=0.8, >=Latex]
        % 定义样式
        \tikzstyle{signal} = [thick, mainblue]
        \tikzstyle{spectrum} = [thick, mainblue, fill=mainblue!10]
        \tikzstyle{axis} = [->, gray]
        ...
        \begin{scope}[xshift=-5cm, yshift=3cm]
            ...
            \draw[signal] plot[domain=-2:2, samples=50] (\x, {0.8*cos(50*\x) + 0.3*cos(120*\x)});
            \node at (0, -0.8) {\small (a) 模拟信号};
        \end{scope}
        
        % 箭头：转换
        \draw[<->, thick, gray] (-0.8, 3.5) -- (0.8, 3.5);
        ...
        \begin{scope}[xshift=5cm, yshift=3cm]
            ...
            \node[below] at (1.2,0) {$f_H$};
            \node at (0, -0.8) {\small (b) 信号频谱};
        \end{scope}

        % --- 运算符号 ---
        \node[font=\bfseries\large, text=textgreen] at (-5, 1.8) {$\times$ (相乘)};
        \node[font=\bfseries\large, text=textgreen] at (5, 1.8) {$*$ (卷积)};
        ...
        \begin{scope}[xshift=-5cm, yshift=0cm]
            ...
            \node[below, text=red] at (-0.5,0) {$-T_s$};
            \node at (0, -0.8) {\small (c) 冲激序列};
        \end{scope}
        
        \draw[<->, thick, gray] (-0.8, 0.5) -- (0.8, 0.5);
        ...
        \begin{scope}[xshift=5cm, yshift=0cm]
            ...
            % 标注带宽 f_s
            \draw[<->, red, dashed] (0, 0.6) -- (1, 0.6) node[midway, above] {$f_s$};
            \node at (0, -0.8) {\small (d) 脉冲频谱};
        \end{scope}

        % --- 等号 ---
        \draw[double, double distance=2pt, gray] (-5, -1.2) -- (-5, -1.8);
        \draw[double, double distance=2pt, gray] (5, -1.2) -- (5, -1.8);
        ...
        \begin{scope}[xshift=-5cm, yshift=-3.5cm]
            \draw[axis] (-2.5,0) -- (2.5,0) node[right] {$t$};
            \draw[axis] (0,-0.5) -- (0,1.5) node[above] {$m_s(t)$};
            % 包络虚线
            \draw[dashed, gray] plot[domain=-2.2:2.2, samples=50] (\x, {0.8*cos(50*\x) + 0.3*cos(120*\x)});
            % 抽样点
            \foreach \x in {-2,-1.5,...,2} {
                \draw[->, thick, mainblue] (\x,0) -- (\x, {0.8*cos(50*\x) + 0.3*cos(120*\x)});
                \fill[mainblue] (\x, {0.8*cos(50*\x) + 0.3*cos(120*\x)}) circle (1.5pt);
            }
            \node[right, font=\footnotesize] at (0.2, 1.2) {$m(nT_s)$};
            \node at (0, -0.8) {\small (e) 抽样信号波形};
        \end{scope}
        
        \draw[<->, thick, gray] (-0.8, -3) -- (0.8, -3);

        % 频域 M_s(f)
        \begin{scope}[xshift=5cm, yshift=-3.5cm]
            \draw[axis] (-2.5,0) -- (2.5,0) node[right] {$f$};
            \draw[axis] (0,-0.5) -- (0,1.5) node[above] {$M_s(f)$};
            ...
            % 滤波器框
            \draw[thick, dashed, red] (-0.5, -0.1) rectangle (0.5, 1.4);
            \node[red, font=\footnotesize] at (0.7, 1.3) {低通滤波};
            ...
            % 混叠条件标注
            \node[draw, rounded corners, fill=gray!10, font=\bfseries\small] at (1.8, 1.8) {$f_s \ge 2f_H$};
            
            \node at (0, -0.8) {\small (f) 周期延拓频谱};
        \end{scope}
