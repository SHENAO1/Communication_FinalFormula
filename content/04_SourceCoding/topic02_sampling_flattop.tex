...
\begin{kbox}{平顶抽样谱与孔径效应推导}
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            axis lines=middle, 
            xmin=-0.2, xmax=2.8, ymin=0, ymax=1.4, 
            xlabel={频率 $f$}, ylabel={幅度 $|H(f)|$},
            xtick={1, 2}, xticklabels={$\frac{1}{\tau}$, $\frac{2}{\tau}$},
            ytick={1}, yticklabels={$A\tau$},
            every axis x label/.style={at={(current axis.right of origin)}, anchor=west},
            every axis y label/.style={at={(current axis.above origin)}, anchor=south},
            samples=200, domain=0.01:2.6, clip=false 
        ]
            \addplot [thick, mainblue] {abs(sin(deg(pi*x))/(pi*x))};
            \draw[dashed, gray, thick] (axis cs:0, 1) -- (axis cs:2.6, 1);
            \node[gray, font=\small] at (axis cs: 2.0, 1.1) {理想保持};
    ...
    \textbf{现象}：抽样脉冲宽度 $\tau$ 越大，Sinc 函数主瓣越窄，高频衰减越严重。

    \tcblower

    \textbf{对策：频率补偿 (均衡)}
    
    为了无失真恢复信号，需在接收端低通滤波器之前，串联一个传输函数为 $H_{eq}(f)$ 的线性修正网络（均衡器），以抵消 $H(f)$ 的影响。
    
    \begin{equation}
        H_{eq}(f) = \frac{1}{H_{Hold}(f)} = \frac{1}{\Sa(\pi f \tau)}
    \end{equation}
    ...
    \begin{tikzpicture}[auto, >=Latex, node distance=1.2cm]
        \tikzstyle{block} = [draw, rectangle, minimum height=2.8em, minimum width=3em, align=center, fill=white]
        
        % 节点定义
        \node (input) {};
        % [修改] 将距离从 0.8cm 增加到 1.6cm，从而拉长第一个箭头
        \node [block, right=1.6cm of input, draw=red, thick, dashed, line width=1pt] (eq) {$\displaystyle \frac{1}{H(f)}$};
        \node [block, right=1.2cm of eq] (lpf) {低通\\滤波器};
        \node [right=1.0cm of lpf] (output) {};
        
        % 连线
        \draw [->, thick] (input) -- node [above, font=\small] {$M_H(f)$} (eq);
        \draw [->, thick] (eq) -- node [above, font=\small] {$M_s(f)$} (lpf);
        \draw [->, thick] (lpf) -- node [above, font=\small] {$M(f)$} (output);
        
        % 辅助标注
        \node [below=0.1cm of eq, red, font=\scriptsize] {倒数滤波器};
        \node [below=0.1cm of lpf, gray, font=\scriptsize] {截止频率 $f_H$};
    \end{tikzpicture}
    \end{center}
    
    \textbf{原理}：输入信号 $M_H(f)$ 先经过倒数滤波器 $1/H(f)$ 变成理想抽样谱 $M_s(f)$（平坦），再通过低通滤波器滤除高频分量，最终得到原始信号 $M(f)$。
\end{kbox}